...
~~~
En el moment que tots els fils intenten fer una reserva però ja no hi ha més places disponibles, 
aquests es queden en espera (wait()). Un cop passa això, no es pot executar el notifyAll() 
que es troba a eliminaReserva().
~~~
...
Al donar una probabilitat mes alata al fer reserves l'execució arriba al limit molt 
abans ja que els fils fan reserves de forma mes freqüent.
~~~
...
En aquest cas l'execució es molt més llarga ja que hi ha major disponibilitat de 
places, per la manca de reserves i un cop es dona el cas el molt probable que es doni una cancel·lació.
~~~
~~~~bash
...
~~~~
El problema principal radica en el fet que no es podria saber quin dels fils ha fet la cancel·lació o la reserva. A més,
en el mètode d’eliminació, cal validar si aquest assistent té una reserva per permetre-li cancel·lar-la.
~~~~